#  This code is copied from https://github.com/vqv/ggbiplot because
#Bioconductor does not support remote dependencies from GitHub.
#  ggbiplot.r
#
...
#  library(grid)
  ...
  df.v$angle <- with(df.v, (180/pi) * atan(df.v$yvar / df.v$xvar))
  df.v$hjust = with(df.v, (1 - varname.adjust * sign(df.v$xvar)) / 2)

  # Base plot
  g <- ggplot(data = df.u, aes(x = df.u$xvar, y = df.u$yvar)) +
  ...
  if(var.axes) {
    ...
    g <- g +
      geom_segment(data = df.v,
                   aes(x = 0, y = 0, xend = df.v$xvar, yend = df.v$yvar),
  ...
  if(var.axes) {
    g <- g +
      geom_text(data = df.v,
                aes(label = df.v$varname, x = df.v$xvar, y = df.v$yvar,
                    angle = df.v$angle, hjust = df.v$hjust),
